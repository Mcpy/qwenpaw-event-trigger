...
(function () {
  ...
  var Table = antd.Table, Tag = antd.Tag, Button = antd.Button, Switch = antd.Switch,
      ...
      Radio = antd.Radio, Alert = antd.Alert, Typography = antd.Typography;
  ...
  function fill(tpl, o) { return tpl.split("{title}").join(o.title || "").split("{event}").join(o.event || ""); }

  /* ================= demo templates =================
     Params use __NAME__ placeholders; server re-validates on register. */
  var TEMPLATES = [
    {
      id: "btc", name: "BTC 阈值监控(滞回)", color: "orange",
      params: [
    ...
    {
      id: "http", name: "HTTP 探测(非2xx/超时触发)", color: "blue",
      params: [
    ...
    {
      id: "file", name: "文件变化监测", color: "purple",
    ...
    {
      id: "port", name: "端口存活(不可达触发)", color: "cyan",
      params: [
    ...
    {
      id: "log", name: "日志关键字(增量扫描)", color: "red",
      params: [
  ...
  function RuleFormModal(props) {
    ...
    var f = React.useState(function () { return antd.Form.useForm()[0]; })[0];
    var src = React.useState(editing ? "path" : "template");
    var source = src[0], setSource = src[1];
    var tpl = React.useState(TEMPLATES[0]);
    var tplId = tpl[0], setTplId = tpl[1];
    var params = React.useState({});
    var pv = params[0], setParams = params[1];
    ...
    function collect() {
      ...
      var body = {
        ...
        share_session: !!v.share_session, tool_safety: v.tool_safety !== false,
      ...
      if (source === "template") {
        ...
        body.script_path = v.script_path;
      }
      return body;
    }
    ...
    function submit(validateOnly) {
      ...
      if (source === "paste" && !v.script_content) { message.error("请粘贴脚本内容"); return; }
      if (source === "path" && !v.script_path) { message.error("请填写脚本路径"); return; }
      setSaving(true);
      var body = collect();
      var p = editing ? ("/" + editing.id + "?validate_only=" + validateOnly) : ("/?validate_only=" + validateOnly);
      api(p, { method: editing ? "PUT" : "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) })
        .then(function (d) {
          var w = (d.warnings || []).filter(function (x) { return x.indexOf("validate-only") < 0; });
          message.success(validateOnly ? "校验通过" + (w.length ? ":" + w.join(";") : "") : "已保存");
    ...
    var tplTabs = e(Tabs, {
      items: [
        { key: "template", label: "从模板", children: e("div", null,
            e(Select, {
              style: { width: "100%", marginBottom: 8 }, value: tplId,
              onChange: function (v) { setTplId(v); },
              options: TEMPLATES.map(function (t) { return { value: t.id, label: t.name }; })
            }),
            TEMPLATES.find(function (t) { return t.id === tplId; }).params.map(function (pr) {
              return e("div", { key: pr.k, style: { marginBottom: 6 } },
                e(Text, { type: "secondary", style: { fontSize: 12 } }, pr.label),
                e(Input, {
                  placeholder: pr.d, value: pv[pr.k] || "",
                  onChange: function (ev) { var n = {}; n[pr.k] = ev.target.value; setParams(Object.assign({}, pv, n)); }
                }));
            })
          ) },
        { key: "paste", label: "粘贴脚本", children: e(TextArea, {
            rows: 10, value: f.getFieldValue("script_content"),
            placeholder: "Python 脚本:读 EVENT_STATE,stdout 输出 {triggered:true, title, event, state}",
            onChange: function (ev) { f.setFieldsValue({ script_content: ev.target.value }); }
          }) },
        { key: "path", label: "引用路径", children: e(Input, {
            placeholder: "/abs/path/checker.py(需绝对路径)", value: f.getFieldValue("script_path"),
            onChange: function (ev) { f.setFieldsValue({ script_path: ev.target.value }); }
          }) }
      ],
      activeKey: source, onChange: setSource
    });
    ...
    return e(Modal, {
      ...
      e("div", { style: { maxHeight: "62vh", overflow: "auto", paddingRight: 4 } },
        e("div", { style: { fontWeight: 600, margin: "4px 0" } }, "① 基本信息"),
        e("div", { style: { display: "flex", gap: 8 } },
          e(Input, { placeholder: "规则名称(必填)", value: f.getFieldValue("name"), onChange: function (ev) { f.setFieldsValue({ name: ev.target.value }); } }),
          e(Space, { align: "center" }, e(Text, null, "启用"), e(Switch, { defaultChecked: !editing || editing.enabled, onChange: function (v) { f.setFieldsValue({ enabled: v }); } }))
        ),
        e("div", { style: { fontWeight: 600, margin: "12px 0 4px" } }, "② 检查器"),
        tplTabs,
        e("div", { style: { display: "flex", gap: 12, alignItems: "center", marginTop: 8 } },
          e(Text, null, "检查间隔(秒,≥10)"),
          e(InputNumber, { min: 10, defaultValue: 60, style: { width: 110 }, onChange: function (v) { f.setFieldsValue({ interval: v }); } })
        ),
        e("div", { style: { fontWeight: 600, margin: "12px 0 4px" } }, "③ 动作"),
        e(Radio.Group, { defaultValue: editing ? editing.action : "agent", onChange: function (ev) { f.setFieldsValue({ action: ev.target.value }); } },
          e(Radio.Button, { value: "notify" }, "通知(不推理)"),
          e(Radio.Button, { value: "agent" }, "Agent 推理")),
        e("div", { style: { marginTop: 8 } },
          e(TextArea, { rows: 2, placeholder: "notify 模板(可插值 {title} {event})", value: f.getFieldValue("notify_template"), onChange: function (ev) { f.setFieldsValue({ notify_template: ev.target.value }); } }),
          e(TextArea, { rows: 2, style: { marginTop: 6 }, placeholder: "agent prompt 模板(可插值 {title} {event})", value: f.getFieldValue("prompt_template"), onChange: function (ev) { f.setFieldsValue({ prompt_template: ev.target.value }); } }),
          e("div", { style: { display: "flex", gap: 8, marginTop: 6 } },
            e(Input, { style: { width: 160 }, defaultValue: "console", addonBefore: "频道", onChange: function (ev) { f.setFieldsValue({ channel: ev.target.value }); } }),
            e(Input, { style: { width: 160 }, defaultValue: "default", addonBefore: "用户", onChange: function (ev) { f.setFieldsValue({ user_id: ev.target.value }); } }),
            e(Input, { placeholder: "会话ID(空=独立会话)", onChange: function (ev) { f.setFieldsValue({ session_id: ev.target.value }); } })
          ),
          e("div", { style: { marginTop: 6, display: "flex", gap: 16 } },
            e(Space, { align: "center" }, e(Text, null, "静默(只跑不投)"), e(Switch, { onChange: function (v) { f.setFieldsValue({ silent: v }); } })),
            e(Space, { align: "center" }, e(Text, null, "工具自动审批"), e(Switch, { defaultChecked: true, onChange: function (v) { f.setFieldsValue({ tool_safety: v }); } }))
          )
        ),
        e("div", { style: { fontWeight: 600, margin: "12px 0 4px" } }, "④ 高级"),
        e("div", { style: { display: "flex", gap: 12, flexWrap: "wrap" } },
          e("label", null, "冷却(秒)", e(InputNumber, { min: 0, defaultValue: 600, style: { width: 100 }, onChange: function (v) { f.setFieldsValue({ cooldown: v }); } })),
          e("label", null, "脚本超时", e(InputNumber, { min: 1, defaultValue: 60, style: { width: 90 }, onChange: function (v) { f.setFieldsValue({ script_timeout: v }); } })),
          e("label", null, "推理超时", e(InputNumber, { min: 1, defaultValue: 120, style: { width: 90 }, onChange: function (v) { f.setFieldsValue({ timeout: v }); } }))
        ),
        e(Alert, {
          style: { marginTop: 10 }, type: "info", showIcon: false,
          message: "保存会走注册关卡:语法检查 → 试跑(dry-run,真实执行一次并初始化状态)→ 内容 hash 锚定。修改脚本即重新校验。"
        })
      ),
  ...
  function RunsDrawer(props) {
    ...
    React.useEffect(function () {
      if (rule) { api("/" + rule.id + "/runs?limit=200").then(function (d) { setRuns(d.runs || []); }); }
    }, [rule, showAll]);

    var shown = (runs || []).slice().reverse().filter(function (r) { return showAll || r.kind === "trigger" || r.kind === "error"; });

    return e(Drawer, {
      open: !!rule, onClose: onClose, width: 460,
      title: rule ? "执行记录:" + rule.name : ""
    },
      e("div", { style: { marginBottom: 8 } },
        e(Space, { align: "center" }, e(Text, null, "显示全部(含 check/skipped)"), e(Switch, { checked: showAll, onChange: setShowAll }))),
      e("div", null, shown.map(function (r, i) {
  ...
  function RulesPage() {
    ...
    var editing = s2[0], setEditing = s2[1];
    var s3 = React.useState(false);
    var modalOpen = s3[0], setModalOpen = s3[1];
    var s4 = React.useState(null);
    var runsRule = s4[0], setRunsRule = s4[1];

    function load() { api("/").then(function (d) { setRules(d.rules || []); }).catch(function (err) { message.error(String(err.message || err)); }); }
    React.useEffect(function () { load(); var t = setInterval(load, 15000); return function () { clearInterval(t); }; }, []);

    function toggle(rule, v) {
      api("/" + rule.id + (v ? "/enable" : "/disable"), { method: "POST" })
    ...
    function del(rule) {
      api("/" + rule.id, { method: "DELETE" }).then(load).catch(function (err) { message.error(String(err.message || err)); });
    }
    ...
    var columns = [
      ...
      { title: "间隔", dataIndex: "interval_seconds", width: 80, render: function (v) { return v + "s"; } },
      { title: "运行/触发", width: 110, render: function (_, r) { return e(Text, null, "⟳" + r.run_count, " ▲", r.trigger_count); } },
      { title: "上次运行", dataIndex: "last_run_at", width: 160, render: function (v) { return e(Text, { type: "secondary" }, ts(v)); } },
      { title: "启用", dataIndex: "enabled", width: 70, render: function (v, r) { return e(Switch, { checked: v, onChange: function (x) { toggle(r, x); } }); } },
      { title: "操作", width: 250, render: function (_, r) {
          return e(Space, { size: 4 },
            e(Button, { size: "small", onClick: function () { runNow(r); } }, "▶ 立即执行"),
            ...
            e(Popconfirm, { title: "删除规则?", onConfirm: function () { del(r); } },
    ...
    return e("div", { style: { padding: 20 } },
      ...
      e(Table, {
        ...
        expandable: {
          rowExpandable: function () { return true; },
      ...
      e(RuleFormModal, {
        ...
        onClose: function () { setModalOpen(false); }, onSaved: function () { setModalOpen(false); load(); }
      }),
  ...
  console.log("[event-trigger] frontend registered");
})();
