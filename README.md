...
- 🛡 **注册关卡**:语法检查 → 试跑(真实执行一次并初始化状态)→ 内容 hash 锚定。野脚本不运行、改脚本拒跑、注册即校验
- 🌊 **防抖**:规则级冷却 + 脚本级滞回(state 由引擎持久化并回传),持续型条件不打风暴
- 🎯 **两种动作**:`notify`(纯通知,零 token)/ `agent`(**进程内注入推理**——与 cron 同走 `stream_query`,无 HTTP/SSE 开销)
- 📬 **收件箱**:运行结果写入收件箱,投递失败自动兜底(cron 同款 trace 机制)
- 📡 **分发模式**:`stream`(逐事件实时转发)/ `final`(仅完整结果);支持静默投递
- 🌍 **中英双语**:UI(跟随控制台语言)/ agent 技能 / 模板元信息
- 📦 **五个内置模板**:股价阈值(NVDA via Yahoo)/ HTTP 探测 / 文件变化 / 端口存活 / 日志关键字(增量扫描)
